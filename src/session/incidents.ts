...
import { setTargetAbsLat } from './racecraft/paths';
...
export function rollMistake(entry: Entry, session: RaceSession): void {
  ...
  const runWide = (seconds: number): void => {
    if (!corner) return;
    entry.avoidT = Math.max(entry.avoidT, seconds);
    entry._avoidWith = 'driver-mistake';
    entry._avoidSide = -corner.side;
  };
...
export function lapWear(entry: Entry, session: RaceSession): void {
  ...
  for (const key of ['e', 'h', 'c'] as const satisfies readonly PartKey[]) {
    if (entry.rel[key] < 0.32 && random() < (0.32 - entry.rel[key]) * 0.55) {
      if (key === 'e') {
...
export function stepRecovery(entry: Entry, session: Session, step: number): void {
  ...
  const loose = error > 0.34 - 0.08 * wet ||
    Math.abs(car.r) > 0.8 - 0.15 * wet || Math.abs(car.slipR) > 0.18 - 0.04 * wet;
  if (loose && car.spd >= 22) {
    const amount = Math.max(
      clamp((error - 0.25) / 0.55, 0, 1),
      clamp((Math.abs(car.r) - 0.60) / 1.2, 0, 1));
    const gather = 1 + 0.6 * wet;
