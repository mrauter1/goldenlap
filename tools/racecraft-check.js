...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    const tracks = GL.BUILT.map(B => {
      ...
      let maxOff = 0, startMax = 0, pitMax = 0, shaped = 0, strict = 0;
      ...
      return {
        ...
        markerMaxError, isolatedMarkerMaxError,
        markerFailures: markers.filter(marker => marker.error > 0.75)
      };
    });
  ...
  for (const t of result.tracks) {
    ...
    const trajectory = t.markerMaxError <= 0.75 && t.soloGrass === 0 && t.soloFinite;
    ...
    console.log(`${t.name}: center ${t.centerLap.toFixed(3)}s · line ${t.lineLap.toFixed(3)}s · ` +
      ...
      `marker error ${t.markerMaxError.toFixed(2)}m (isolated ${t.isolatedMarkerMaxError.toFixed(2)}m) · ` +
      `solo lat ${t.soloMaxLat.toFixed(2)}m grass ${t.soloGrass} heading ${t.soloMaxHeading.toFixed(2)} · ` +
      `${faster && faded && bounded && corners && trajectory ? 'OK' : 'FAIL'}`);
    if (!trajectory) {
