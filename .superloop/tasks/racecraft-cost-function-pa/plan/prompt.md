...
## Required outputs
Update `.superloop/tasks/racecraft-cost-function-pa/plan/plan.md` as the single source of truth for the plan, including milestones, interface definitions, and risk register details in that one file.

Create or update `.superloop/tasks/racecraft-cost-function-pa/plan/phase_plan.yaml` as the canonical machine-readable ordered phase decomposition. If the task is genuinely small and coherently shippable as one slice, produce exactly one explicit phase rather than inventing artificial decomposition.

Also append a concise entry to `.superloop/tasks/racecraft-cost-function-pa/plan/feedback.md` with what changed and why.
...
5. Do not edit `.superloop/tasks/racecraft-cost-function-pa/plan/criteria.md` (verifier-owned).
